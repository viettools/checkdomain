...
def whois_via_web(USER_AGENT, domain, domain_type):
    ...
    if domain_type:
        ...
        if domain_type == '.com.jo':
            ...
            ddl = '11'
        elif domain_type == '.agri.jo':
            ddl = '13'
        elif domain_type == '.tv.jo':
            ddl = '14'
        elif domain_type == '.fm.jo':
            ddl = '15'
        elif domain_type == '.ai.jo':
            ddl = '16'
        # 12 .الاردن
    if domain:
    ...
    if req_get and req_get.status_code == 200 and req_get.text:
        ...
        if raw_data:
            ...
            if next_view_state_details and next_view_state_generator_details and next_event_validation_details:
                ...
                try:
                    req_post = requests.post('https://www.dns.jo/FirstPageen.aspx', data=payload, headers=headers, cookies=req_cookie, verify=False)
                except:
                    pass

                if req_post and req_post.status_code == 200 and req_post.text and req_post.text.find('Organization Name') > -1:
                    req_get = req.get('https://www.dns.jo/WhoisDetails.aspx', headers=headers, cookies=req_post.cookies.get_dict(), verify=False)
                    if req_get and req_get.status_code == 200:
                        raw_data = req_get.text
                        if raw_data:
                            creation_date_details = parse_jo_data('<span id="ContentPlaceHolder1_Regdatev" style="font-weight:bold;">(.*?)</span>', raw_data)
                            if creation_date_details:
                                result.append('Creation Date: {0}'.format(creation_date_details))
                            
                            for i in range(1, 11):
                                ns_details = parse_jo_data('<span id="ContentPlaceHolder1_Server{0}V" style="font-weight:bold;">(.*?)</span>'.format(i), raw_data)
                                if ns_details:
                                    result.append('Name Server: {0}'.format(ns_details))
                            
                            admin_details = parse_jo_data('<span id="ContentPlaceHolder1_AdminDetailsV" style="font-weight:bold;">(.*?)</span>', raw_data)
                            if admin_details:
                                result.append('AdminOrganization: {0}'.format(admin_details))
                            
                            tech_details = parse_jo_data('<span id="ContentPlaceHolder1_TechLabelV" style="font-weight:bold;">(.*?)</span>', raw_data)
                            if tech_details:
                                result.append('TechOrganization: {0}'.format(tech_details))
                                
                            billing_details = parse_jo_data('<span id="ContentPlaceHolder1_BillingDetailsV" style="font-weight:bold;">(.*?)</span>', raw_data)
                            if billing_details:
